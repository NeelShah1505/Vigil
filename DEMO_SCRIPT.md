# VIGIL — 5-Minute Demo Video Script
...
| Time | Screen | Say / do |
|---|---|---|
| 0:00–0:20 | Dashboard idle (PCR 0%, red) | "AI agents can now pay per request with x402. But an agent that can pay isn't an agent that can *stay* solvent. This is Vigil: working capital for autonomous agents." |
| 0:20–0:50 | Architecture diagram (README) | "A metered API behind x402, a Vigil agent with its own treasury, a liquidity router, and a Hedera Consensus Service audit trail. Everything settles on testnet — verify every claim on HashScan." |
...
| 1:10–1:40 | Forecast + shortfall events appear | "Vigil forecasts the shortfall *before* it happens: it needs 10.3 FUSDC including custom fees and buffer. Shortfall detected — logged to HCS." |
| 1:40–2:10 | Route decision table renders | "Now the treasury-desk moment: it evaluates liquidity routes — the VigilRouter swap at 0.3 percent, SaucerSwap, unavailable for this token on testnet. It picks the cheapest and executes. Two on-chain legs, both verified." |
...
| 4:40–5:00 | Dashboard, closing | "Agents shouldn't just know how to pay. They should know how to stay solvent. Vigil." |
